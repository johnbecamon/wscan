#!/usr/bin/env python3
"""WSCAN v14 — Mobile App"""
import threading, re, os, json, socket, random, hashlib
...
def prog(c, t):
    if PROG_CB: Clock.schedule_once(lambda dt: PROG_CB(c, t))

def stats(v, d, a, s):
    if STATS_CB: Clock.schedule_once(lambda dt: STATS_CB(v, d, a, s))

def status(t, c=None):
    if STATUS_CB: Clock.schedule_once(lambda dt: STATUS_CB(t, c))
...
uploads/ upload/ files/ media/ images/ assets/ static/ public/
...
demo master owner super staff helpdesk tech service system developer dev
engineer api apiuser web mobile mysql postgres database sa dba oracle
admin1 admin2 Admin1 Admin2 admin123 Admin123 wp-admin wpuser wp
""".split()))
...
www mail ftp webmail admin administrator cpanel dev dev1 dev2 dev3 stage
staging test testing beta alpha demo sandbox api api1 api2 api-v1 api-v2
rest graphql app apps mobile blog forum shop wiki support help docs cdn
static assets media files upload img images us eu asia ph sg hk jp kr
srv server host node web01 web02 web1 web2 db database mysql vpn sso auth
login monitor ns1 ns2 dns git svn repo jenkins ci cd build jira
...
def is_real(t, path):
    ...
    for ph in ["page not found","404 not found","does not exist","no such file","access denied","forbidden"]:
...
def run_vuln(base):
    ...
    status("Scanning vulnerabilities...", CYAN)
    bl = get_base(base)
    found = []
...
def run_dir(base):
    ...
    status("Brute-forcing directories...", ACCENT)
    bl = get_base(base)
    hits = []
    for i, w in enumerate(PATHS, 1):
        ...
        col = GREEN if r.status_code == 200 else YELLOW if r.status_code in (301,302) else RED if r.status_code == 403 else CYAN
        log(f"  [{r.status_code}] {r.url}", col)
        if note: log(f"     {note}", TEXT_DIM)
        hits.append({"code": r.status_code, "url": r.url, "note": note})
    if not hits: log("  ✓ Walang extra paths", GREEN)
    return hits

def find_logins(base, hits):
    cands = []
    for h in hits:
        if h["code"] in (200,401) and re.search(r"(login|signin|admin|auth|panel|dashboard|wp-login|administrator)", h["url"], re.I):
            cands.append(h["url"])
    for p in ["admin/","admin/login.php","administrator/","wp-login.php","login","login.php","admin.php","user/login"]:
...
def verify(url, html, uf, pf, u, p):
    ...
    s = requests.Session()
    fetch(url, session=s)
    r = fetch(target, method=method, data={uf: u, pf: p}, allow_redirects=False, session=s)
    if not r: return False, "req failed"
    ...
    for path in ["admin/","wp-admin/","dashboard/","panel/"]:
        ...
        if not rr: continue
        if rr.status_code == 200 and not re.search(r'<input[^>]*type=["\']password["\']', rr.text, re.I):
...
def run_admin(base, hits, max_creds=300):
    ...
    status("Brute-forcing admin...", YELLOW)
    ...
    for url in cands:
        ...
        for u, p in [(a,b) for a in USERS for b in PWDS]:
            ...
            rr = fetch(url, method="POST", data={uf_n: u, pf_n: p}, allow_redirects=False)
            if not rr: continue
            body = rr.text or ""
            cand = False
...
def run_subs(base):
    ...
    if re.match(r"^\d+\.\d+\.\d+\.\d+$", host):
        log("  [!] IP target", YELLOW); return []
    parts = host.split(".")
    rd = ".".join(parts[-2:])
    scheme = "https" if base.startswith("https") else "http"
    found = []
    log(f"🌐 Subdomain scan ({len(SUBS)})", (0.5,0.4,1,1))
    status("Finding subdomains...", (0.5,0.4,1,1))
    for i, sub in enumerate(SUBS, 1):
        ...
        for sch in (scheme, "https" if scheme=="http" else "http"):
            ...
            if r:
                ...
                break
    if not found: log("  ✓ Walang subdomain", GREEN)
    return found

def run_wp(base):
    log("📝 WordPress deep scan", (0.4,0.6,1,1))
    status("Scanning WordPress...", (0.4,0.6,1,1))
    found = []
...
def run_all(url, mode, max_creds, outdir):
    ...
    if mode in ("full","wp"):
        wp = run_wp(base)
        stats(len(vulns), len(dirs), len(admin), len(subs))
...
class WScanApp(App):
    def build(self):
        ...
        url_card.add_widget(Label(text="📍  Target URL", font_size=sp(13),
                                   bold=True, color=TEXT, size_hint_y=None,
        ...
        mb = BoxLayout(orientation="vertical", size_hint_x=0.6, spacing=dp(2))
        mb.add_widget(Label(text="Mode", font_size=sp(11), color=TEXT_DIM,
        ...
        mb.add_widget(self.mode_spinner)
        row.add_widget(mb)
        xb = BoxLayout(orientation="vertical", size_hint_x=0.4, spacing=dp(2))
        xb.add_widget(Label(text="Max creds", font_size=sp(11),
                             color=TEXT_DIM, size_hint_y=None, height=dp(14)))
        ...
        sr = BoxLayout(orientation="horizontal", size_hint_y=None,
        ...
        sr.add_widget(self.stop_btn)
        cb = RoundButton(text="[b]🗑  CLEAR[/b]", markup=True,
                          bg=(0.28,0.32,0.40,1), fg=(1,1,1,1), font_size=sp(13))
        cb.bind(on_release=self.clear_log)
        sr.add_widget(cb)
        btn_card.add_widget(sr)
        root.add_widget(btn_card)

        sc = Card(orientation="vertical", size_hint_y=None, height=dp(80),
        ...
        sc.add_widget(self.status_lbl)
        self.progress = ProgressBar(max=100, value=0,
                                     size_hint_y=None, height=dp(14))
        sc.add_widget(self.progress)
        root.add_widget(sc)

        cc = Card(orientation="vertical", size_hint_y=None, height=dp(400),
                   padding=dp(10), spacing=dp(6))
        cc.add_widget(Label(text="📜  Live Output", font_size=sp(13),
                             bold=True, color=TEXT, size_hint_y=None,
                             height=dp(20), halign="left", valign="middle"))
        cc.children[-1].bind(size=lambda l, v: setattr(l, "text_size", (l.width, None)))
        cscroll = ScrollView(size_hint_y=None, height=dp(345))
        ...
        cscroll.add_widget(self.console_box)
        cc.add_widget(cscroll)
        self.console_scroll = cscroll
        root.add_widget(cc)
        ...
        self._log("🎯 Piliin ang mode, tapos START", TEXT_DIM)
    ...
    def _prog(self, c, t):
        self.progress.value = (c / t * 100) if t else 0
    ...
    def start_scan(self, *a):
        ...
        if not url:
            ...
            self._status("Walang URL", RED); return
        ...
        self._status("Starting...", CYAN)
        outdir = "/storage/emulated/0/wscan_output"
        threading.Thread(target=self._run, args=(url, mode, mc, outdir),
                          daemon=True).start()
    ...
    def _run(self, url, mode, mc, outdir):
        try: run_all(url, mode, mc, outdir)
            ...
            Clock.schedule_once(lambda dt: self._reset(), 0)
